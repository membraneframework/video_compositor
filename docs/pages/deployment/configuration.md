...
Output framerate for all output streams. This value can be a number or string in format `NUM/DEN`, where both `NUM` and `DEN` are unsigned integers.
...
:::warning
This option does not apply to logs produced by a `FFmpeg` or embeded Chromium instance that is used for web rendering.
:::
...
Enable web rendering capabilities. With this option disabled, you can not use [`WebView` components](./components/WebView) or register [`WebRenderer` instances](./renderers/web).

### `LIVE_COMPOSITOR_WEB_RENDERER_GPU_ENABLE`

Disable GPU support inside embedded Chromium instance.
