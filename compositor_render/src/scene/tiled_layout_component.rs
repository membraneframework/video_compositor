...
use crate::scene::layout_node::Layout;

use super::{layout_node::LayoutView, Component, NodeParams};

pub struct TiledLayout {
    pub spec: TiledLayoutSpec,
    pub children: Vec<Component>,
}

impl TiledLayout {
    pub fn layout(&self, children: Vec<NodeParams>) -> (Layout, Vec<NodeParams>) {
        let optimal_rows_cols = optimize_inputs_layout(children.len() as u32, &self.spec);
        let tile_size = tile_size(&optimal_rows_cols, &self.spec);
        let tile_layouts = layout_tiles(
            children.len() as u32,
            &optimal_rows_cols,
            tile_size,
            &self.spec,
        ...
        let mut layout = Layout {
            elements: vec![LayoutView {
                background_color: self.spec.background_color_rgba,
                top: 0.0,
                left: 0.0,
                width: self.spec.resolution.width as f32,
                height: self.spec.resolution.height as f32,
                rotation: 0.0,
                child_index: None,
            }],
            resolution: self.spec.resolution,
        };
        let children = layout.embed_children(tile_layouts, children);
        (layout, children)
    }
}
